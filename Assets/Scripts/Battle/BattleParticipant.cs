...
public class BattleParticipant : MonoBehaviour
{
    ...
    public void ChangeHealth(float amount)
    {
        ...
        if (amount < 0f)
        {
            ...
            _hitEffectCoroutine = StartCoroutine(HitEffect(-amount));
        }
        if (Health <= 0) Die();
    ...
    {
        float damageFactor = damage / Level.maxHealth;
        float time = Mathf.Clamp01(0.5f * damageFactor + 0.3f);
        Color target = Color.Lerp(Color.white, Color.red, damageFactor + 0.3f);

        sr.color = Color.white;
        float t = 0;
        while (t < time / 2f)
        {
            sr.color = Color.Lerp(Color.white, target, t * (1f / time));
            t += Time.deltaTime;
            yield return null;
        }
        t = 0;
        while (t < time / 2f)
        {
            sr.color = Color.Lerp(target, Color.white, t * (1f / time));
            t += Time.deltaTime;
            yield return null;
        }
        sr.color = Color.white;
